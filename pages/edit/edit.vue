<template>
    <view class="whole">
        <view class="mine">
            <view class="my">
                <view class="my_basic">
                    <image :src="userInfo.my_image_url" mode="aspectFit"></image>
                    <!-- <text click="editUsername">{{ userInfo.user_name }}</text> -->
					<text v-if="!isEditing" @click="startEditing">{{ userInfo.user_name }}</text>
					<input class="my_basic_input" v-if="isEditing" v-model="editedUserName" @blur="saveUserName" placeholder= "输入新名字">
                </view>
...
function saveInformation() {
    ...
    uni.request({
        ...
        method: 'POST',
        data: { ...userInfo }, // 展开运算符来复制userInfo对象
        success(res) {
